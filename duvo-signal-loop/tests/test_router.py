...
class TestLeadEmailFor:
    ...
    def test_existing_plus_tag_replaced_not_stacked(self):
        ...
        assert (
            router.lead_email_for("jakub+old@gmail.com", "albert.cz") == "jakub+albert-cz@gmail.com"
        )
...
class TestToolSchema:
    ...
    async def test_tool_names_are_correct(self):
        ...
        async def capture_run_agent(
            system, user, tools, impls, max_turns=8, final_tools=(), log=None
        ):
            captured["tool_names"] = {t.name for t in tools}
            captured["impl_names"] = set(impls.keys())

        with patch("duvo.agents.router.router.run_agent", side_effect=capture_run_agent):
...
class TestDryRunConfidentTier1:
    ...
    async def test_crm_status_dry_run_string(self, monkeypatch):
        ...
        with patch(
            ...
            side_effect=_fake_run_agent_factory(
    ...
    async def test_no_real_crm_called_in_dry_run(self, monkeypatch):
        ...
        with (
            patch(
                "duvo.agents.router.router.run_agent",
                side_effect=_fake_run_agent_factory(["crm_upsert"]),
            ),
    ...
    async def test_provider_names_reflect_monkeypatched_values(self, monkeypatch):
        ...
        with patch(
            "duvo.agents.router.router.run_agent",
            side_effect=_fake_run_agent_factory(["crm_upsert", "outreach_queue"]),
        ):
            await router.run_router(rr, dry_run=True)
...
class TestSafetyGuards:
    ...
    async def test_slack_alert_refuses(self, tier, needs_human_research):
        ...
        with patch("duvo.agents.router.router.run_agent", side_effect=capturing_run_agent):
            await router.run_router(rr, dry_run=True)
...
class TestRealMode:
    ...
    async def test_crm_upsert_calls_real_crm_adapter(self):
        ...
        with (
            ...
            patch(
                "duvo.writeback.crm.upsert_account", AsyncMock(return_value=sentinel)
            ) as mock_upsert,
        ):
    ...
    async def test_slack_real_mode_confident_t1_calls_adapter(self):
        ...
        with (
            patch(
                "duvo.agents.router.router.run_agent",
                side_effect=_fake_run_agent_factory(["slack_alert"]),
            ),
            patch(
                "duvo.writeback.slack.alert_tier1", AsyncMock(return_value=sentinel)
            ) as mock_alert,
        ):
            await router.run_router(rr, dry_run=False)
    ...
    async def test_outreach_real_mode_confident_t1_calls_adapter(self):
        ...
        with (
            patch(
                "duvo.agents.router.router.run_agent",
                side_effect=_fake_run_agent_factory(["outreach_queue"]),
            ),
            patch(
                "duvo.writeback.outreach.queue_lead", AsyncMock(return_value=sentinel)
            ) as mock_queue,
        ):
