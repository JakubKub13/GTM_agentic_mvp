...
`.codex/skills/cmux-virtual-ide/scripts/cmux-ide.sh`. It derives the repo root from
...
1. **Read the intent from the user's request** (Codex does not substitute
   `$ARGUMENTS` inside a skill, so infer the target from the natural-language ask):
...
2. **Start or reopen** (the default action):
   ...
   ```bash
   .codex/skills/cmux-virtual-ide/scripts/cmux-ide.sh --start
   ```

   For a subpath:

   ```bash
   .codex/skills/cmux-virtual-ide/scripts/cmux-ide.sh --start writeback
   ```

3. **Relay the printed `IDE URL:` line to the user verbatim.** With no subpath it is:

   ```text
   http://127.0.0.1:8787/?folder=/Users/phantomghost/Desktop/GTM-duvo/duvo-signal-loop
   ```
...
4. **Stop** — only when explicitly asked:
   ...
   ```bash
   .codex/skills/cmux-virtual-ide/scripts/cmux-ide.sh --stop
   ```

5. **Status / verify** — when they just want to know if it's up:

   ```bash
   .codex/skills/cmux-virtual-ide/scripts/cmux-ide.sh --status
   ```
